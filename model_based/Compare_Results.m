...
hold off
 ...
 %% old
    ...
    Data_Iskander      = xlsread('Data_1__Farrage_filtered.xlsx');   
    Data_Farrage       = xlsread('Data_1__Farrage_filtered.xlsx'); 

    
    Time_Is        = Data_Iskander(:,1);

    Pos4_Is        = Data_Iskander(:,2);    
    V4_Is          = Data_Iskander(:,3);

    Theta_1_Is     = Data_Iskander(:,4);
    DTheta_1_Is    = Data_Iskander(:,5);
    Theta_2_Is     = Data_Iskander(:,6);
    DTheta_2_Is    = Data_Iskander(:,7);

 
    Time_Fa        = Data_Farrage(:,1);

    Pos4_Fa        = Data_Farrage(:,2);    
    V4_Fa          = Data_Farrage(:,3);

    Theta_1_Fa     = Data_Farrage(:,4);
    DTheta_1_Fa    = Data_Farrage(:,5);
    Theta_2_Fa     = Data_Farrage(:,6);
    DTheta_2_Fa    = Data_Farrage(:,7);

%%

figure(1)
% fig('fontsize',16,'width',18,'height',14,'units','centimeters')
        
    subplot(2,1,1)
    % plot(Time_Diff,H_BoomPoints,'r.','MarkerSize',20);
    plot(Time_Is-12.45,Pos4_Is-37.45,'r','LineWidth',2)
    hold on
    plot(Time_Fa-12,Pos4_Fa+133,'blue','LineWidth',2)
    ylabel({'$\theta_4$','[deg]'},'interpreter','latex')
    legend({'Data Driven','Polynomial'},'interpreter','latex',...
    ...
    subplot(2,1,2)
    plot(Time_Is-12,V4_Is,'r','LineWidth',2)
    hold on
    plot(Time_Fa-12,V4_Fa,'b','LineWidth',2)
    ylabel({'$\dot\theta_4$','[deg/$sec$]'},'interpreter','latex')
...
figure(2)
    ...
    subplot(2,2,1)
    plot(Time_Is-12.45,Theta_1_Is,'r','LineWidth',2)
    hold on
    plot(Time_Fa-12,-Theta_1_Fa,'b','LineWidth',2)
    ylabel({'$\theta_1$','[deg]'},'interpreter','latex')
    legend({'Data Driven','Polynomial'},'interpreter','latex',...
    'Location','SouthWest','Orientation','Vertical','Fontsize',10) 
 
    subplot(2,2,3)
    plot(Time_Is-12.45,DTheta_1_Is,'r','LineWidth',2)
    hold on
    plot(Time_Fa-12,DTheta_1_Fa,'b','LineWidth',2)
    ylabel({'$\dot\theta_1$','[deg/$sec$]'},'interpreter','latex')

    subplot(2,2,2)
    plot(Time_Is-12.45,Theta_2_Is,'r','LineWidth',2)
    hold on
    plot(Time_Fa-12,-Theta_2_Fa,'b','LineWidth',2)
    ylabel({'$\theta_2$','[deg]'},'interpreter','latex')
 
    subplot(2,2,4)
    plot(Time_Is-12.45,DTheta_2_Is,'r','LineWidth',2)
    hold on
    plot(Time_Fa-12,-DTheta_2_Fa,'b','LineWidth',2)
    ylabel({'$\dot\theta_2$','[deg/$sec$]'},'interpreter','latex')
%%
    
    % xlswrite('Farrage_trajectory_Simulation.xlsx',[OPT_T_new(:),OPT_Pos4_new(:),OPT_V4_new(:),OPT_Acc4_new(:),OPT_Theta_1_new(:),OPT_DTheta_1_new(:),OPT_Theta_2_new(:),OPT_DTheta_2_new(:)],'Sheet1','A2');
    % xlswrite('Farrage_verif_load_sway_Simulation.xlsx',[t_Con(:),Th1_Con(:),DTh1_Con(:),Th2_Con(:),DTh2_Con(:)],'Sheet1','A1');
