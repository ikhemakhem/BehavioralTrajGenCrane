...
%
% Loads the bundled experimental crane data, truncates the data matrix using
% precomputed simulation hyperparameters, then runs a grid search over
% (mu, sigma, lambda) to find the best trajectory-generation parameters.
% Finally generates and plots the optimal trajectory using the best
% parameters from data/trajopt_grid_search_long.mat.
%
...
simParams = load(fullfile(dataDir, 'sim_grid_search_real_data_500.mat'));
...
%% ----------------------------- Grid search ----------------------------
diary(fullfile(scriptDir, 'grid_search_opt_traj_log.txt'));

mu_values     = custom_spaced_values(1e-1,  50, 5, 6);
sigma_values  = custom_spaced_values(1e-1,  50, 5, 7);
...
save(fullfile(dataDir, 'trajopt_grid_search_real_data.mat'), ...
...
save(fullfile(scriptDir, 'opt_traj_result.mat'), 'w_opt', 'w_model', 'w_dd');
...
for k = 1:5
    ...
    p3  = plot(time, w_opt(k:q:end),    '-',  'LineWidth', 1.5, 'Color', color_opt); %#ok<NASGU>
